...
uint8_t OLED_I2C_WaitAck(void)
{
    ...
    // 读取应答信号
    if (HAL_GPIO_ReadPin(OLED_SDA_PORT, OLED_SDA_PIN) == GPIO_PIN_RESET) {
        ack = 0; // 收到应答
    } else {
        ack = 1; // 未收到应答
    }
...
void OLED_SendCmd(uint8_t cmd)
{
    static uint8_t sendBuffer[2] = {0x00, 0x00}; // 命令模式
    sendBuffer[1] = cmd;
    ...
    OLED_I2C_SendByte(sendBuffer[0]);
    OLED_I2C_WaitAck();
    OLED_I2C_SendByte(sendBuffer[1]);
    OLED_I2C_WaitAck();
